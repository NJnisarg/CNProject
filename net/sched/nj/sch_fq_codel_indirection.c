...
struct fq_codel_sched_data {
	...
	// $$
	u32     *hashtable; /* The hashtable that holds the indexes to the actual flows table entries */
...
// $$
void cuckoo_rehash(const struct fq_codel_sched_data *q,
                   struct sk_buff *skb, int value_to_insert)
{
    int temp_index;
    for(int i=0;i<(q->flows_cnt)/2;i++){
...
/*
 * The cuckoo hash returns the physical flow index in the flows table.
 * This number is 1-indexed(starting from 1 and not 0)
 */
// $$
                                         ...
                                         struct sk_buff *skb)
{
    ...
    {
        q->hashtable[hash1] = ++(q->last_used_flow);
    ...
    if(q->hashtable[hash1] != 0 && q->hashtable[hash2]==0)
    {
        ...
        if(skb_get_hash(q->flows[idx].head)==skb_get_hash(skb))
            ...
            q->hashtable[hash2] = ++(q->last_used_flow);
     ...
     */
    int value_to_insert = ++(q->last_used_flow);
...
static void fq_codel_reset(struct Qdisc *sch)
{
	...
	// $$
    memset(q->hashtable, 0, q->flows_cnt * sizeof(u32));
	sch->q.qlen = 0;
			 ...
			 struct netlink_ext_ack *extack)
{
	...
	if (!q->flows) {
		...
		// $$
        q->hashtable = kvcalloc(q->flows_cnt, sizeof(u32), GFP_KERNEL);
        if (!q->hashtable) {
            err = -ENOMEM;
            goto alloc_failure;
        }
